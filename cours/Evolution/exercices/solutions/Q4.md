...
- Le code dans chaque package est refactoré pour enlever la duplication
- Le type de refactoring utilisé dans chaque cas est indiqué

# Solution suggérée

## Package `one`

La méthode `doExercise()` est commune, nous pouvons donc la "monter" dans `Exercise`.
Il reste de la duplication, par exemple la gestion du poids dans `Benchpress` et `Deadlift`, qui
peut être combinée dans une nouvelle classe `WeightedExercise`, et nous pouvons aussi créer
une nouvelle classe `UnweightedExercise` pour les deux autres.
...
Tout d'abord, au lieu de recréer des `Scanner`s, nous pouvons créer une seule instance.
Ensuite, vu que chaque méthode a une structure similaire, nous pouvons extraire ce comportement.
Finalement, nous pouvons utiliser des constantes pour le texte des questions.
...
Les deux classes ont des méthodes similaires, nous pouvons donc créer une nouvelle classe contenant ces méthodes,
et utiliser la composition dans les classes existantes.
...
La classe utilise trois manière différente d'approximer pi, mais le résultat est quasiment le même,
nous pouvons donc ne garder que la manière la plus simple, et l'extraire dans une méthode séparée.

Évidemment, dans une vraie application, nous pourrons stocker la valeur dans une variable au lieu de la re-calculer,
ou simplement utiliser `Math.PI`.
